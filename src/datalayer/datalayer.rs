...
pub fn load_and_process_history(
    ...
    for i in 0..df.height() {
        if let (Some(w), Some(l), Some(s)) = (winners.get(i), losers.get(i), surfaces.get(i)) {
            ...
            // Stats aus der CSV holen (oder 0.0, falls das Match vor 1991 war und keine Stats hat)
            ...
            // ==========================================
            // DAS NEUE KI-FEATURE: SERVE EDGE (Match-Up)
            // ==========================================
            // Wie gut ist der Aufschlag von P1 gegen den Return von P2?
